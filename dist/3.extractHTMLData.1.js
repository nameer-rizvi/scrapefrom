...
function extractHTMLData1(config, $) {
    ...
    for (let i = 0; i < extractConfigs.length; i++) {
        const { name: explicitName, delimiter: localDelimiter, selector, attribute, json: extractJSON, extract: extractChild, extracts: extractChildren, filter: jsonFilter, keyPath: jsonKeyPath, } = extractConfigs[i];
        let name, delimiter;
